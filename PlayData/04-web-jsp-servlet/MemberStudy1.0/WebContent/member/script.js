...
var addresserror = "주소 양식을 전부 채워주세요";
var emailconfirmerror = "인증에 실패했습니다 \n 인증번호를 다시 확인해주세요";
var emailconfirm = "인증에 성공했습니다 \n 회원가입을 계속 진행해주세요";
var confirmemailerror = "이메일 인증을 진행해주세요";
var input_ok = "회원가입에 성공하셨씁미다!!! \n 잉에얀야오에오에ㅑ농야ㅗ에오 \n 이제 막 만족하냥 이임마막?"
...
function emailcheck(email1, email2) {
	...
	open(url,"emailcheck window", "toolbar=no, scrollbar=no, status=no, menubar=no, width=500px, height=300px")
}

function zipcheck() {
	var url = "zipCheck.jsp";
	open(url,"zipcheck window","toolbar=no, scrollbar=no, status=no, menubar=no, width=500px, height=300px");
}
function zipfocus() {
	zipform.area3.focus();
}
...
function inputcheck() {
	...
	if(!inputform.id.value) {
		...
		return false;
	} else if(!inputform.zipcode1.value || !inputform.zipcode2.value 
			|| !inputform.address1.value || !inputform.address2.value) {
		alert(addresserror);
...
//		return false; 	// submit 처럼 넘어가라는 기능이 원래 달려있는게 아니어서 넣어줄 필요가 없다.
		...
		open(url,"confrim window" ,"toolbar=no, scrollbar=no, status=no, menubar=no, width=500px, height=300px");
	}
